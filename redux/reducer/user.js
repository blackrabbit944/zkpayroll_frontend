...
export const BEFORE_SWITCH_LANGUAGE = 'BEFORE_SWITCH_LANGUAGE';
...
export function switchLanguage(lang) {
    return {
        ...
        types: [BEFORE_SWITCH_LANGUAGE, SWITCH_LANGUAGE_SUCCESS, SWITCH_LANGUAGE_FAILED],
        // 检查缓存 (可选):
        shouldCallAPI:  (state) => !state.getIn(['user','is_switching_language']),
        // 进行取：
        callAPI: () => {
            return httpRequest({
                'method'  : 'PATCH',
                'url'     : '/v1/user/switch_language',
                'data'    : {
                    lang
                }
            })
        ...
        data_format : (result) => {
            var output = normalize(result.data, userSchema)
            return output
        },
...
export function loginUserUseWallet(condition) {

    let source_from = getCookie('source_from');
    
    if (source_from) {
        condition['source_from'] = source_from;
    }
...
export function registerUserUseWallet(condition) {
    return {
        // 要在之前和之后发送的 action types
        types: [BEFORE_REGISTER, REGISTER_SUCCESS, REGISTER_FAILED],
        // 检查缓存 (可选):
        shouldCallAPI:  (state) => !state.getIn(['user','is_reigster_ing']),
        // 进行取：
        callAPI: () => {
...
export function reducer(state = Immutable.fromJS({
    ...
    'is_reigster_ing'   : false,
    'is_login_ing'      : false,
    'is_logout_ing'     : false,
    'is_updating'       : false,
    'is_switching_language' : false
}), action) {
    switch (action.type) {
        ...
        case BEFORE_SWITCH_LANGUAGE:
            return state.set('is_switching_language',true);

        case SWITCH_LANGUAGE_SUCCESS:
        case SWITCH_LANGUAGE_FAILED:
            return state.set('is_switching_language',false);
